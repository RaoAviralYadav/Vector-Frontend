# Vectr

A modern landing page built with Next.js and TypeScript for a product or brand called **Vectr**.

## Overview

Vectr is a clean, responsive marketing site that showcases product benefits, process steps, standards, FAQs, and calls to action. The project uses the Next.js App Router and a component-driven structure to keep the landing page modular and easy to maintain.

## Features

- Hero section with branded messaging
- Responsive site header and navigation
- Feature icons and feature grid sections
- Process stepper to explain workflows
- Standards or trust section
- FAQ section
- Call-to-action footer
- Intro loader animation
- Reusable `Button` component in `components/ui/button.tsx`

## Tech Stack

- Next.js
- React
- TypeScript
- PostCSS
- pnpm

## Getting Started

### Prerequisites

- Node.js 18+ (or compatible)
- `pnpm`

### Install

```bash
pnpm install